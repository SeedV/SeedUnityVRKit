...
namespace Mediapipe.Unity.Holistic
{
  public class HolisticTrackingGraph : GraphRunner
  {
    ...
    public SidePacket BuildSidePacket(int inputRotation, bool hFlip, bool vFlip) {
      var sidePacket = new SidePacket();

      sidePacket.Emplace("input_rotation", new IntPacket(inputRotation));
      sidePacket.Emplace("input_horizontally_flipped", new BoolPacket(hFlip));
      sidePacket.Emplace("input_vertically_flipped", new BoolPacket(vFlip));
